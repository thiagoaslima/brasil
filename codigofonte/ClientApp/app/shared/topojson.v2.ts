...
function object(topology, o) {
    ...
    function geometry(o): featureObj {
...
var neighbors = function (objects) {
    ...
    for (var i in indexesByArc) {
        let _iNumber = parseInt(i, 10);
        for (var indexes = indexesByArc[i], m = indexes.length, j = 0; j < m; ++j) {
            for (var k = j + 1; k < m; ++k) {
                var ij = indexes[j], ik = indexes[k], n;
                if ((n = neighbors[ij])[i = bisect(n, ik).toString()] !== ik.toString()) n.splice(i, 0, ik);
                if ((n = neighbors[ik])[i = bisect(n, ij).toString()] !== ij.toString()) n.splice(i, 0, ij);
            }
        }
    }
